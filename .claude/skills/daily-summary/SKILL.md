...
## Step 4 — Check Granola and Fathom for un-captured meetings
Find meetings that happened on the target day but don't yet have a note in `5. Meetings/`:
1. List the target day's meetings from **both sources**:
   - **Granola** — `mcp__claude_ai_Granola__list_meetings` (or `mcp__claude_ai_Granola__query_granola_meetings`); keep those whose date is the target day.
   - **Fathom** — `mcp__claude_ai_Fathom__list_meetings` (bracket the day with `created_after`/`created_before`, or pass `max_pages` and filter); keep those whose date is the target day, capturing each recording's `recording_id`, `url`, `recorded_by`, and `calendar_invitees`.

   If one source is unavailable or empty, say so briefly and continue with the other; if neither returns anything, skip to Step 5.
2. **Deduplicate across sources, then against existing notes.** First merge the two source lists into one set of distinct meetings — the same meeting often appears in both Granola and Fathom; treat two entries as the same meeting when they share the target date and a matching title (compare case-insensitively, ignoring punctuation) or clearly overlapping attendees. Keep one entry per real meeting, remembering which source(s) it came from. Then glob `5. Meetings/` for existing notes dated the target day (`<YYYY-MM-DD> - <Title>.md`); a meeting is **un-captured** if no existing note has the same date and a matching title (treat a clear title match as captured even if not identical).
3. Present the un-captured meetings as a short list (noting each one's source) and ask whether to create notes for them. Do not create notes the user didn't confirm. For each confirmed meeting, write `5. Meetings/<YYYY-MM-DD> - <Title>.md` and update the Meetings MOC, following the matching skill's conventions:
   - **Granola-sourced** → follow **`meeting-from-granola`**; you already have the meeting ID, so call `mcp__claude_ai_Granola__get_meetings` with it directly (no URL needed).
